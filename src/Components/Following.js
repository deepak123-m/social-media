...
const Following = () => {
  ...
  if(localStorage.getItem("token") === null)
  {
    ...
    const render = (name,userName,id,image,following,followers) => {
          ...
          {
            return(  <Link to="/profile" state={{img:image,nme:name,unme:userName,flw:following,flwrs:followers,id:id,yes:"no"}}>
    ...
    <div>
       <div className="   rounded-2xl mx-8  border-gray-800 border ">
        <div className="text-white">
         ...
         {alluser &&
                    alluser.map(({ name, userName,_id,image,following,followers}) => {
                      let userid = _id;
                      return render(name,userName,userid,image,following,followers)
                    }
                )}
